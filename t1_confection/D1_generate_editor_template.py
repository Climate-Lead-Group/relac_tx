...
def create_editor_template(data, output_path):
    ...
    demand_desc = [
        ...
        "",
        "DATA SOURCES:",
        "  1. 'Generacion electrica por fuente - Anual - OLADE.xlsx' (Sheet 1.2023, Row 21)",
        "  2. Trade balance data (if TradeBalanceDemandAdjustment = YES)",
        "  Reference year: 2023 (from cell A4)",
        "",
        "FORMULA (with trade balance enabled):",
        "  Base_Demand = Generation_OLADE(PJ) + Imports(PJ) - Exports(PJ)",
        "  Demand(year) = Base_Demand x (1 + growth_rate x (year - 2023))",
        "",
        "FORMULA (without trade balance):",
        ...
        "",
        "NOTE: Trade balance is incorporated into the base demand at the reference year.",
        "  Imports and exports for future years use forward-fill (last available year as constant).",
    ]
    ...
    lower_desc = [
        ...
        "",
        "DATA SOURCES:",
        "  1. Projected demand from A-O_Demand.xlsx (trade-adjusted if enabled)",
        "  2. 'Renewability_Targets' sheet - Target renewable % by year (optional)",
        "  3. 'Technology_Weights' sheet - Custom tech distribution (optional)",
        "",
        "FORMULA:",
        "  Generation(year) = Demand(year) + Exports(PJ) - Imports(PJ)",
        "  LowerLimit(tech,year) = Generation(year) x Share_normalized(tech,year)",
        "",
        "  When trade balance is disabled: LowerLimit = Demand x Share (no trade adjustment).",
        "",
        "SHARE CALCULATION:",
    ...
    method_desc = [
        "Calculates TotalTechnologyAnnualActivityLowerLimit from generation and normalized shares.",
        "Only applies when ActivityLowerLimitFromOLADE = YES.",
        "",
        "CALCULATION PIPELINE:",
        "  Step 1: Calculate demand with trade in base:",
        "          Base_Demand = Gen_OLADE + Imports_PJ - Exports_PJ",
        "          Demand(year) = Base_Demand × (1 + growth_rate × (year - ref_year))",
        "  Step 2: Derive generation from projected demand:",
        "          Generation(year) = Demand(year) + Exports_PJ - Imports_PJ",
        "  Step 3: Read % renewable from Renewability_Targets (with interpolation)",
        "  Step 4: Distribute % renewable among renewable fuels using OLADE generation weights",
        "  Step 5: Distribute % non-renewable among non-renewable fuels using OLADE generation weights",
        "  Step 6: Normalize all shares year-by-year to ensure Σ(shares) = 1.0",
        "  Step 7: For non-base scenarios, override years 2023-2025 with base scenario shares",
        "  Step 8: LowerLimit = Generation × Share for each technology/year",
        "",
        "  When trade is disabled: Generation = Demand (no trade adjustment), and demand",
        "  is read from A-O_Demand.xlsx as fallback.",
        "",
        "REQUIREMENTS:",
        "  - ActivityLowerLimitFromOLADE must be YES",
        "  - DemandFromOLADE must be YES (demand must be computed first)",
    ...
    trade_desc = [
        "Incorporates trade balance into demand/generation and sets TRN interconnection limits.",
        ...
        "",
        "PART 1 - DEMAND & GENERATION CALCULATION:",
        "  Trade is incorporated into the base demand from the start (not as a post-hoc adjustment).",
        "  Step 1: Base_Demand = Gen_OLADE + Imports_PJ - Exports_PJ (at reference year)",
        "  Step 2: Demand(year) = Base_Demand × (1 + growth_rate × (year - ref_year))",
        "  Step 3: Generation(year) = Demand(year) + Exports_PJ - Imports_PJ",
        "  The generation values feed into ActivityLowerLimit calculation.",
        "  Conversion: 1 GWh = 0.0036 PJ",
        "  For years beyond available trade data, uses last available year as constant (forward-fill).",
        "",
    ...
    instructions = [
        ...
        ["  * If TradeBalanceDemandAdjustment = YES: Demand = Gen + Imports - Exports", ""],
        ...
        ["  * Generation is derived: Generation = Demand + Exports - Imports", ""],
        ["  * Formula: LowerLimit[tech,year] = Generation[country,year] × Share_normalized[tech,year]", ""],
