...
def write_scenario(name, src, apply_nli, shares, ren_2030):
    ...
    dst = src.parent / f"Pre_processed_{name}_0_StorageDelayN5_OpenBCK_RMCarefulXLSX_FLOORED_VEGCON.txt"
...
def main():
    ...
    for name in ("INV", "VGB"):
        dst = SCENARIOS[name][0].parent / f"Pre_processed_{name}_0_StorageDelayN5_OpenBCK_RMCarefulXLSX_FLOORED_VEGCON.txt"
        if not dst.exists():
            continue
