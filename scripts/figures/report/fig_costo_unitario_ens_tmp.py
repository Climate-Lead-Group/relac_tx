"""
fig_costo_unitario_ens_tmp.py — SCRIPT TEMPORAL. Variante de
fig_costo_unitario.py que SÍ incluye la Energía no Suministrada (ENS) en el
costo unitario, sumada DENTRO del componente "Combustible" (sin leyenda
nueva: la figura sigue mostrando CAPEX anualizado / O&M / Combustible).

Diferencias de cómputo respecto a fig_costo_unitario.py:
...
Uso:
    python scripts/figures/report/fig_costo_unitario_ens_tmp.py
    python scripts/figures/report/fig_costo_unitario_ens_tmp.py --years 2030 2040 2050
    python scripts/figures/report/fig_costo_unitario_ens_tmp.py --scenarios BAC ISR --ref BAC
    python scripts/figures/report/fig_costo_unitario_ens_tmp.py --den-sin-bck
"""
...
def build_figure(data: dict, scenarios: list[str], years: list[int], ref: str = "BAC"):
    ...
    rs.apply_report_style(ax, "Costo unitario del sistema incl. ENS [USD/MWh]", european_y=True)
...
def main(argv: list[str] | None = None):
    ...
    base = args.out or os.path.join(FIGURES_DIR, "fig_costo_unitario_ens_tmp")
